module Jekyll
	class PremiumSample < Page
		def initialize(site, base, dir, post)
			...
			self.process(@name)
			self.read_yaml(File.join(base, '_layouts'), 'premium_sample.html')
	...
	class PremiumSampleGenerator < Generator
		priority :low
		def generate(site)
			dir = 'premium_sample'
			site.posts.docs.each do |post|
				site.pages << PremiumSample.new(site, site.source, File.join(dir, post.id), post)
			end
		end
	end
end